...
public class AppGPPServiceHost : ServiceBase
{
    ...
    private void Log(string message)
    {
        ...
        {
            // Ignora falha secundária de log.
        }
    }
...
function Ensure-ServiceHostBinary {
  param([string]$InstallDir)

  $source = Get-SourceText
  $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
  $serviceHostPath = Join-Path $InstallDir ("scripts\AppGPP-ServiceHost-$stamp.exe")
  $serviceHostSourcePath = Join-Path $InstallDir "scripts\AppGPP-ServiceHost.cs"
  Write-Step "Compilando host do servico AppGPP..."
  Set-Content -LiteralPath $serviceHostSourcePath -Value $source -Encoding UTF8

  $compilerArgs = @{
    TypeDefinition = $source
    Language = 'CSharp'
    OutputAssembly = $serviceHostPath
  ...
  Add-Type @compilerArgs
  return $serviceHostPath
}

$serviceHostPath = Ensure-ServiceHostBinary -InstallDir $InstallDir
Write-Step "Host do servico pronto em: $serviceHostPath"
Write-Output $serviceHostPath
