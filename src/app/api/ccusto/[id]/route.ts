...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!canAccess) {
            return NextResponse.json({ message: 'Sem permissão para acessar centro de custo' }, { status: 403 });
        }
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!canUpdate) {
            return NextResponse.json({ message: 'Sem permissão para alterar centro de custo' }, { status: 403 });
        }
        const { id } = await params;
