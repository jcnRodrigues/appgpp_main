...
type FuncionariosTransferencia = {
    idF: string;
    idMatFun: string;
    nomeFun: string;
...
type ItemTransferencia = {
    funcionario: FuncionariosTransferencia;
    custoDestino: string;
    salvando: boolean;
};

export default function TransferenciaCustoFuncionariosTable() {
    const { data: session } = useSession();
    const formularios = ((session?.user as any)?.formularios || []) as string[];
    const canPrint = hasModuleActionPermission(formularios, 'FUNCIONARIOS', 'PRINT');
    const canUpdate = hasModuleActionPermission(formularios, 'FUNCIONARIOS', 'UPDATE');

    const [dados, setDados] = useState<FuncionariosTransferencia[]>([]);
    ...
    const buscarPorMatricula = async () => {
        ...
        try {
            ...
            const json = await response.json();
            const encontrado = (json.data || []).find((f: FuncionariosTransferencia) => f.idMatFun === matriculaBusca.trim()) || (json.data || [])[0] || null;
            if (!encontrado) {
    ...
    const executarTransferencia = async (idF: string) => {
        ...
        try {
            ...
            const response = await fetch(`/api/funcionarios/${idF}`, {
                method: 'PUT',
            ...
            const resultadoTransferencia: FuncionariosTransferencia = {
                ...item.funcionario,
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="table-surface overflow-x-auto rounded-2xl border border-border/60 bg-[#10191b] shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
                <table className="w-full min-w-[900px]">
                    <thead className="bg-gray-50 border-b">
                        <tr>
                            <th className="px-4 py-3 text-left text-xs font-semibold">Matricula</th>
