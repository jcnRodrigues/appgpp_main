...
interface FuncionariosTableProps {
    funcionarios?: FuncionariosItem[];
}

export default function FuncionariosTable({ funcionarios: initialFuncionarios }: FuncionariosTableProps) {
    ...
    const [funcionarios, setFuncionarios] = useState<FuncionariosItem[]>(initialFuncionarios || []);
    ...
    useEffect(() => {
        const carregarOpcoes = async () => {
            try {
                const response = await fetch('/api/funcionarios/opcoes');
                if (!response.ok) {
    ...
    const handleDelete = async (idF: string) => {
        ...
        try {
            const response = await fetch(`/api/funcionarios/${idF}`, { method: 'DELETE' });
            if (response.ok) {
    ...
    return (
        <div className="table-surface space-y-4">
            <div className="sticky top-[calc(var(--app-header-height)+70px)] z-30 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 pb-1">
                <div className="rounded-2xl border border-border/60 bg-[#10191b] p-2 shadow-[0_20px_60px_rgba(0,0,0,0.22)] space-y-2">
                ...
                <table className="w-full min-w-[1000px] table-auto">
                    <thead>
                        <tr>
                            ...
                            <th className="w-[24%] bg-gray-50 px-3 py-3 text-left text-xs md:text-sm font-semibold text-gray-900">Nome</th>
                            <th className="w-[8%] bg-gray-50 px-3 py-3 text-left text-xs md:text-sm font-semibold text-gray-900">CPF</th>
                            <th className="w-[16%] bg-gray-50 px-3 py-3 text-left text-xs md:text-sm font-semibold text-gray-900">Função</th>
                            <th className="w-[8%] bg-gray-50 px-3 py-3 text-left text-xs md:text-sm font-semibold text-gray-900">Data Admissão</th>
                            <th className="w-[6%] bg-gray-50 px-3 py-3 text-left text-xs md:text-sm font-semibold text-gray-900">Status</th>
                            <th className="w-[8%] bg-gray-50 px-3 py-3 text-left text-xs md:text-sm font-semibold text-gray-900">Ações</th>
                        </tr>
                    </thead>
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={funcionario.idF} className="border-b hover:bg-gray-50 transition">
                                ...
                                <td className="px-3 py-4 text-xs md:text-sm text-gray-700">
                                    {funcionario.dataAdmFun ? new Date(funcionario.dataAdmFun).toLocaleDateString('pt-BR') : '-'}</td>
                                <td className="px-3 py-4 text-xs md:text-sm">
                                    <span
                                        className={`px-3 py-1 rounded-full text-xs text-[9px] font-semibold 
                                    ${getStatusBadgeClass(funcionario.tbStatusFun?.descricaoStatusFun)}`}>
                                        {funcionario.tbStatusFun?.descricaoStatusFun || '-'}
                                    </span>
                                </td>
                                <td className="px-2 py-2.5 text-xs md:text-sm flex items-center justify-center">
                                    <div className="flex items-center gap-2 md:gap-2">
                                        <Button asChild
                                            ...
                                            <Link href={`/funcionarios/${funcionario.idF}`}
                                                title="Editar"
