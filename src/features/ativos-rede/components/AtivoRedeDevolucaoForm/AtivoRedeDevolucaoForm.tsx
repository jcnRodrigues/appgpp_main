...
export default function AtivoRedeDevolucaoForm({ ativoRedeId }: { ativoRedeId: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            window.systemAlert?.('sucesso', 'Devolução registrada com sucesso.');
            router.push(`/ativos-rede/${ativoRedeId}`);
        } catch (error) {
            console.error(error);
            window.systemAlert?.('erro', error instanceof Error ? error.message : 'Erro ao registrar devolução');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="bg-background min-h-screen py-6">
            <div className="max-w-4xl mx-auto px-4 sm:px-6">
                <div className="form-title-sticky flex items-center mb-6">
                    ...
                    <div>
                        <h1 className="text-h2 font-bold">Devolução de Ativo</h1>
                        {ativo && (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-2 text-xs text-amber-700">
                                            O centro de custo principal sera vinculado automaticamente a Filial Paraupebas.
                                        </p>
                                    </div>
                                    <div>
