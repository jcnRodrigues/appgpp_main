...
export default function AtivoRedeTransferenciaForm({ ativoRedeId }: { ativoRedeId: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            window.systemAlert?.('sucesso', 'Transferencia registrada com sucesso.');
            router.push(`/ativos-rede/${ativoRedeId}`);
        } catch (error) {
            console.error(error);
            window.systemAlert?.('erro', error instanceof Error ? error.message : 'Erro ao transferir ativo de rede');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="bg-background min-h-screen py-6">
            <div className="max-w-4xl mx-auto px-4 sm:px-6">
                <div className="form-title-sticky flex items-center mb-6">
                    <Link href={`/ativos-rede/${ativoRedeId}`} className="mr-4">
                        <ChevronLeft className="h-6 w-6 text-primary" />
                    </Link>
                    <div>
                        <h1 className="text-h2 font-bold">Transferencia de Ativo</h1>
                        {ativo && (
                            <p className="text-sm text-gray-600">
                                {ativo.codigoAtivoRede} - {ativo.nomeAtivoRede}
                            </p>
                        )}
                    </div>
                </div>
                        ...
                        <>
                            <div className="border-b pb-6">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <h2 className="text-h4 font-bold">Nova Transferencia</h2>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium mb-2">Status apos transferencia</label>
                                        <select
                                ...
                                <div className="mt-4">
                                    ...
                                    <textarea
                                        ...
                                        onChange={handleChange}
                                        placeholder="Informe o motivo ou detalhes da transferencia"
