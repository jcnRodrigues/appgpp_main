...
export default function PatrimonioTable({ patrimonios: initialPatrimonios }: PatrimonioTableProps) {
    ...
    const centroDropdownRef = useRef<HTMLDivElement | null>(null);
    ...
    useEffect(() => {
        setPaginaAtual(1);
    }, [idFiltro, statusSelecionados, centroSelecionado, itensPorPagina]);
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (!centroDropdownRef.current) return;
            if (!centroDropdownRef.current.contains(event.target as Node)) {
    ...
    }, [idFiltro, statusSelecionados, centroSelecionado, paginaAtual, itensPorPagina]);
            ...
            <div className="sticky top-[calc(var(--app-header-height)+96px)] z-30 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 pb-2">
                <div className="bg-white rounded-lg shadow-md p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        />
                        <KeyIcon className="h-4 w-4 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2 pointer-events-none" />


                        <div className="relative" ref={centroDropdownRef}>
                            <button
                                type="button"
                                onClick={() => setCentroDropdownOpen((prev) => !prev)}
