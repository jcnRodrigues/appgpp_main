...
import { getPatrimonioCardById, getTipoPatrimonioById } from "@/backend/service/Patrimonio.services/patrimonio.service";
...
export default async function PatrimonioCard({
    ...
    const tipoPat = await getTipoPatrimonioById(idTipoPat);


    return (
        <Link href={`/patrimonio/${idP}`}>
            <div className="bg-white rounded-3xl p-3 mb-7 relative">
                ...
                <div className="px-4 pt-2 pb-4 text-center mb-3">
                    <h3 className=" text-h4 mb-1 truncate overflow-hidden whitespace-nowrap">
                        {tipoPat?.descricaoTipPat} - {Patrimonio?.idPat}
                    </h3>
                    <p>
