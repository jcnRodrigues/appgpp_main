...
"[project]/src/app/api/ccusto/medicao/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function formatarPercentual(fator) {
...
function obterDetalheDevolucao(patrimonio) {
    ...
    return {
        devolvido: true,
        detalhe: fim ? `Movimentação de devolução: de ${inicio} até ${fim}.` : `Movimentação de devolução iniciada em ${inicio} (sem data de fim).`
    };
}
...
function construirMovimentosPatrimonio(params) {
    ...
    const devolucao = patrimonio.tbDevolucao?.map((d)=>d.dataInicioDevolucao).sort((a, b)=>a.getTime() - b.getTime())[0] || null;
    if (devolucao) {
...
function parseValor(value) {
    ...
    const texto = String(value).trim().replace(/r\$/gi, '').replace(/\s+/g, '').replace(/\./g, '').replace(/,/g, '.').replace(/[^0-9.-]/g, '');
...
async function POST(request) {
    try {
        ...
        if (!allowAll && centros.length > 0 && !centros.includes(idCCusto)) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                message: 'Centro de custo não permitido.'
            }, {
                status: 403
            });
        }
        if (!file || !(file instanceof File)) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                message: 'Arquivo Excel não informado.'
            }, {
                status: 400
            });
        }
        const dataInicioMedicao = typeof dataInicioMedicaoRaw === 'string' && dataInicioMedicaoRaw ? new Date(`${dataInicioMedicaoRaw}T00:00:00`) : null;
        const dataFimMedicao = typeof dataFimMedicaoRaw === 'string' && dataFimMedicaoRaw ? new Date(`${dataFimMedicaoRaw}T23:59:59.999`) : null;
        if (!dataInicioMedicao || Number.isNaN(dataInicioMedicao.getTime())) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                message: 'Data de início da medição inválida.'
            }, {
                status: 400
            });
        }
        if (!dataFimMedicao || Number.isNaN(dataFimMedicao.getTime())) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                message: 'Data de fim da medição inválida.'
            }, {
                status: 400
            });
        }
        ...
        const idIndex = headerIndex.findIndex((h)=>[
                'idpat',
                'patrimonio',
                'id'
            ].includes(h));
        const valorIndex = headerIndex.findIndex((h)=>[
                'valor',
                'valorpat',
                'valorpatrimonio'
            ].includes(h));
        ...
        const patrimoniosCentro = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$back$2d$end$2f$service$2f$Patrimonio$2e$services$2f$patrimonio$2e$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["listarPatrimoniosPorCentroCusto"])(idCCusto, {
            dataInicioMedicao,
            dataFimMedicao,
            dataTransferenciaRef: dataFimMedicao
        });
        const mapaPatrimonios = new Map(patrimoniosCentro.map((p)=>[
                String(p.idPat).toUpperCase(),
                p
            ]));
        const resultados = [];
        ...
        for(let i = 1; i < rows.length; i += 1){
            ...
            const valorSistema = patrimonio.valorRateado ?? patrimonio.valorPat ?? null;
            const valorBase = patrimonio.valorPat ?? null;
            const rateioInfo = patrimonio.rateioInfo;
            ...
            const detalheRateio = rateioInfo && valorBase !== null && valorSistema !== null ? `${formatarMoeda(valorBase)} x ${formatarPercentual(rateioInfo.fator)} (${diasNoCentro}/${diasPeriodo} dias) = ${formatarMoeda(valorSistema)}` : null;
            ...
            const valorOk = valorInformado !== null && valorSistema !== null && Math.abs(valorInformado - valorSistema) <= tolerancia;
            resultados.push({
                linha: i + 1,
                idPat,
                descricaoPat: patrimonio.descricaoPat ?? null,
                matriculaAlocada: (()=>{
                    const transfAloc = patrimonio.tbTransferenciaAlocacao?.[0];
                    const transfCusto = patrimonio.tbTransferenciaCustoPatrimonio?.filter((t)=>t.dataTransferencia.getTime() <= dataFimMedicao.getTime())?.sort((a, b)=>b.dataTransferencia.getTime() - a.dataTransferencia.getTime())[0];
                    ...
                    const custoLabel = transfCusto?.custoDestino ? `${transfCusto.custoDestino.codigoCCusto ? `${transfCusto.custoDestino.codigoCCusto} - ` : ''}${transfCusto.custoDestino.descricaoCCusto || ''}`.trim() : '';
                ...
                dataTransferenciaConsiderada: (()=>{
                    const transfAloc = patrimonio.tbTransferenciaAlocacao?.[0]?.dataTransferencia || null;
                    const transfCusto = patrimonio.tbTransferenciaCustoPatrimonio?.filter((t)=>t.dataTransferencia.getTime() <= dataFimMedicao.getTime())?.sort((a, b)=>b.dataTransferencia.getTime() - a.dataTransferencia.getTime())[0]?.dataTransferencia || null;
                    const ultima = [
                        transfAloc,
                        transfCusto
                    ].filter((d)=>!!d).sort((a, b)=>b.getTime() - a.getTime())[0] || null;
        ...
        const naoInformados = patrimoniosCentro.filter((p)=>!idsArquivo.has(String(p.idPat).toUpperCase())).map((p)=>{
            ...
            return {
                ...
                valorSistema: p.valorRateado ?? p.valorPat ?? null,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__de464ec2._.js.map