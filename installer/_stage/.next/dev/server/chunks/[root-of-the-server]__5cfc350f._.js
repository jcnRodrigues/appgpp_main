...
"[project]/src/app/api/unifi-proxy/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function fetchUiApi(effectiveApiKey, path, options) {
    ...
    const response = await fetch(`https://api.ui.com/v1/${path}`, {
        ...
        body: options?.data && [
            ...
            'PUT',
            'PATCH'
        ].includes(method) ? JSON.stringify(options.data) : undefined
    });
...
async function fetchAllHosts(effectiveApiKey, pageSize) {
    const hosts = [];
    let nextToken = '';
    let page = 0;
    const maxPages = 100;
...
async function fetchAllDevicesByHost(effectiveApiKey, hostId, pageSize) {
    const devices = [];
    let nextToken = '';
    let page = 0;
    const maxPages = 100;
    ...
    do {
        ...
        const flattened = list.length > 0 && Array.isArray(list[0]?.devices) ? list.flatMap((item)=>item.devices || []) : list;
...
function mapToSitesAndDevices(hosts) {
    ...
    const devices = hosts.flatMap((host)=>(Array.isArray(host.devices) ? host.devices : []).map((device, idx)=>({
    ...
    try {
        ...
        if (action === 'hosts-and-devices' || action === 'sites-and-devices') {
            ...
            if (action === 'sites-and-devices') {
                const { sites, devices } = mapToSitesAndDevices(byHost.map((item)=>({
        ...
        if (!path) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'path e necessario'
            }, {
                status: 400
            });
        }
        const cleanPath = String(path).startsWith('/') ? String(path).slice(1) : String(path);
        const url = consoleId ? `https://api.ui.com/v1/connector/consoles/${consoleId}/${cleanPath}` : `https://api.ui.com/v1/${cleanPath}`;
        const fetchOptions = {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5cfc350f._.js.map